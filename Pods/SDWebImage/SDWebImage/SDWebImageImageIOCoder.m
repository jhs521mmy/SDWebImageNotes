...
#pragma mark - Decode ---- 解码
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    switch ([NSData sd_imageFormatForImageData:data]) {//判断是啥图片
        case SDImageFormatWebP:
            // Do not support WebP decoding
            return NO;
        case SDImageFormatHEIC:
            // Check HEIC decoding compatibility
            return [[self class] canDecodeFromHEICFormat];    //检查HEIC的兼容性
...
- (UIImage *)decodedImageWithData:(NSData *)data {
    ...
    image.sd_imageFormat = [NSData sd_imageFormatForImageData:data];// 根据二进制数据判断图片的格式
...
- (nullable UIImage *)sd_decompressedAndScaledDownImageWithImage:(nullable UIImage *)image {
    ...
    if (![[self class] shouldDecodeImage:image]) {
        return image;
    }    //2.检查图像应不应该压缩，原则是：如果图像大于目标尺寸才需要压缩
    ...
    //3.拿到数据信息 sourceImageRef
        CGImageRef sourceImageRef = image.CGImage;
    //4.计算原图的像素 sourceResolution
        CGSize sourceResolution = CGSizeZero;
        sourceResolution.width = CGImageGetWidth(sourceImageRef);
        sourceResolution.height = CGImageGetHeight(sourceImageRef);
    //5.计算原图总像素 sourceTotalPixels
    ...
    //7.计算目标像素 destResolution
        CGSize destResolution = CGSizeZero;
        destResolution.width = (int)(sourceResolution.width*imageScale);
        destResolution.height = (int)(sourceResolution.height*imageScale);
    //8.获取当前的颜色空间 colorspaceRef
        // device color space
        CGColorSpaceRef colorspaceRef = SDCGColorSpaceGetDeviceRGB();
    ...
    //11.计算第一个原图方块 sourceTile，这个方块的宽度同原图一样，高度根据方块容量计算
    ...
    //13.计算原图像方块与方块重叠的像素大小 sourceSeemOverlap
        float sourceSeemOverlap = (int)((kDestSeemOverlap/destResolution.height)*sourceResolution.height);
        CGImageRef sourceTileImageRef;
        // calculate the number of read/write operations required to assemble the
        // output image.
    //14.计算原图像需要被分割成多少个方块 iterations
    ...
    //15.根据重叠像素计算原图方块的大小后，获取原图中该方块内的数据，把该数据写入到相对应的目标方块中
